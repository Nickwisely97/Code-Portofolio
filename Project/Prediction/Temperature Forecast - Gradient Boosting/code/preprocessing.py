...
def run_preprocessing(
    ...
    rows_after = len(df)
    logger.info(f"[Preprocessing] Done: {rows_after}/{rows_before} valid rows.")
...
def split_train_test(
    ...
    logger.info(
        f"Train: {len(df_train)} rows ({df_train['timestamp'].min()} → {df_train['timestamp'].max()})\n"
        f"Test : {len(df_test)} rows ({df_test['timestamp'].min()} → {df_test['timestamp'].max()})"
    )
